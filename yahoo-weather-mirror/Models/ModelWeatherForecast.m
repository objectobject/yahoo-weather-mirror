...
const NSUInteger kHHXXDefaultHourCount = 24;
...
- (id)hhxxWeatherFiltrator:(HHXXWeatherForecastType)type
{
    ...
    switch (type) {
        ...
        {
            [configureModel setObject:self.item.forecast forKey:kHHXXYahooWeatherItemKey_Forecast_10Days];
            NSMutableArray* housInfo = [NSMutableArray arrayWithCapacity:kHHXXDefaultHourCount];
            NSDateFormatter* formatter = [NSDateFormatter new];
            [formatter setDateFormat:@"HH:00"];
            
            for (NSUInteger index = 0; index < kHHXXDefaultHourCount; ++index)
            {
                NSDate* now = [NSDate dateWithTimeIntervalSinceNow:index * 3600];
                ModelForecast* randomForecast = self.item.forecast[arc4random() % [self.item.forecast count]];
                housInfo[index] = @{
                                    kHHXXYahooWeatherItemKey_ForecastHoursTime:[formatter stringFromDate:now],
                                    kHHXXYahooWeatherItemKey_ForecastHoursCode:[randomForecast formatterWeatherIcon],
                                    kHHXXYahooWeatherItemKey_ForecastHoursTemp:[randomForecast.low stringByAppendingString:kHHXXTemperatureUnit]
                                    };
            }
            [configureModel setObject:housInfo forKey:kHHXXYahooWeatherItemKey_Forecast_24Hours];
        }
            break;
